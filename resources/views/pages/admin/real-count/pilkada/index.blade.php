...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="mb-4 d-flex justify-content-start" style="width: 100%">
            <a href="{{ route('real-count.pilkada.export') }}" class="btn btn-success">Export Laporan Real Count
                Pilkada</a>
        </div>
        {{-- Charts --}}
        <div class="col-12 mb-4">
            <div class="card">
                <div class="card-header header-elements">
                    <div>
                        <h5 class="card-title mb-0">Total Suara Pilkada Keseluruhan</h5>
                    </div>
                </div>
                <div class="card-body pt-2">
                    <canvas id="pilkada" class="chartjs" data-height="500"></canvas>
                </div>
            </div>
        </div>
        ...
        <div class="col-12 mb-4">
            <div class="card">
                <div class="card-header header-elements">
                    <div>
                        <h5 class="card-title mb-0">Total Suara Pilkada Berdasarkan Dapil</h5>
                    </div>
                </div>
                <div class="card-body-dapil pt-2">
                    <div class="d-flex justify-content-center gap-3">
                        ...
                        <button type="button" class="btn btn-primary" onclick="changeSuaraPilkada()">Filter</button>
                        <button type="button" class="btn btn-danger d-none"
                            onclick="destroySuaraPilkada()">Destroy</button>
                    </div>
                    <canvas id="pilkada-dapil" class="chartjs" data-height="500"></canvas>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    <script>
        const ctx = document.getElementById('pilkada').getContext('2d');
        ...
        function changeSuaraPilkada() {
            const id = $('select[name=dapil_id]').val();
            const ctx = document.getElementById('pilkada-dapil').getContext('2d');
            ...
            $.ajax({
                url: `/admin/real-count/pilkada/${id}`,
                method: 'GET',
        ...
        function destroySuaraPilkada() {
            $('#pilkada-dapil').remove();
            // destroy chart
